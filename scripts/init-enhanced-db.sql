-- AI Handoff MCP Enhanced Database Schema
-- This script creates the complete database schema including enhanced session monitoring

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Sessions table to track active handoff sessions (enhanced with monitoring fields)
...
-- Context history table to store conversation and context data (enhanced with performance tracking)
...
-- Session lifecycle tracking table
CREATE TABLE IF NOT EXISTS session_lifecycle (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id UUID NOT NULL REFERENCES sessions(id) ON DELETE CASCADE,
    event_type VARCHAR(50) NOT NULL, -- 'created', 'expired', 'archived', 'dormant', 'reactivated', 'status_changed'
...
CREATE TABLE IF NOT EXISTS analytics_aggregations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    aggregation_type VARCHAR(50) NOT NULL, -- 'session_stats', 'handoff_stats', 'performance_trends'
...
-- Insert initial system metric to mark schema creation
INSERT INTO system_metrics (metric_name, metric_value, metric_type, labels)
VALUES ('schema_initialized', 1, 'counter', jsonb_build_object(
    'schema_version', 'enhanced_monitoring_v1',
    'initialized_at', NOW()
));

-- Success message
SELECT 'Enhanced Session Monitoring Database Schema Created Successfully!' as status;