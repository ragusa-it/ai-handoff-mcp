...
export class ResilientDatabaseManager {
  ...
  async initialize(): Promise<void> {
    ...
    const initResult = await errorHandler.handleWithRecovery(
      async () => {
        ...
        structuredLogger.logInfo('Resilient database manager initialized successfully', {
          pgPoolSize: this.pgPool.totalCount,
  ...
  private async initializePostgreSQL(): Promise<void> {
    try {
      ...
      structuredLogger.logInfo('PostgreSQL initialized successfully', {
        maxConnections: this.pgPool.options.max,
        minConnections: this.pgPool.options.min
      });
    } catch (error) {
  ...
  private setupPostgreSQLEventHandlers(): void {
    this.pgPool.on('connect', (client) => {
      this.pgHealth.connected = true;
      this.pgHealth.consecutiveFailures = 0;
      structuredLogger.logInfo('PostgreSQL client connected');
    });
    
    this.pgPool.on('error', (error, client) => {
      this.pgHealth.consecutiveFailures++;
      structuredLogger.logError('PostgreSQL pool error', {
        error: error.message,
    ...
    this.pgPool.on('remove', (client) => {
      structuredLogger.logInfo('PostgreSQL client removed from pool');
    });
  }
  ...
  private setupRedisEventHandlers(): void {
    this.redisClient.on('connect', () => {
      this.redisHealth.connected = true;
      this.redisHealth.consecutiveFailures = 0;
      structuredLogger.logInfo('Redis client connected');
    });
    
    this.redisClient.on('error', (error) => {
      this.redisHealth.consecutiveFailures++;
      structuredLogger.logError('Redis client error', {
        error: error.message,
    ...
    this.redisClient.on('reconnecting', () => {
      structuredLogger.logInfo('Redis client reconnecting');
    });
  ...
  private async performHealthCheck(): Promise<void> {
    ...
    try {
      ...
      if (!this.pgHealth.connected) {
        this.pgHealth.connected = true;
        this.pgHealth.consecutiveFailures = 0;
        structuredLogger.logInfo('PostgreSQL health recovered');
      }
    } catch (error) {
      this.pgHealth.connected = false;
      this.pgHealth.consecutiveFailures++;
      ...
      structuredLogger.logError('PostgreSQL health check failed', {
        error: error instanceof Error ? error.message : String(error),
        consecutiveFailures: this.pgHealth.consecutiveFailures
      });
    }
    ...
    try {
      ...
      if (!this.redisHealth.connected) {
        this.redisHealth.connected = true;
        this.redisHealth.consecutiveFailures = 0;
        structuredLogger.logInfo('Redis health recovered');
      }
    } catch (error) {
      this.redisHealth.connected = false;
      this.redisHealth.consecutiveFailures++;
      ...
      structuredLogger.logError('Redis health check failed', {
        error: error instanceof Error ? error.message : String(error),
        consecutiveFailures: this.redisHealth.consecutiveFailures
      });
  ...
  private async attemptPostgreSQLRecovery(): Promise<void> {
    structuredLogger.logInfo('Attempting PostgreSQL recovery');
    ...
    try {
      ...
      structuredLogger.logInfo('PostgreSQL recovery successful');
    } catch (error) {
      structuredLogger.logError('PostgreSQL recovery failed', {
        error: error instanceof Error ? error.message : String(error)
      });
    }
  }
  ...
  private async tryRedisFailover(): Promise<void> {
    ...
    for (let i = 0; i < this.redisBackupClients.length; i++) {
      try {
        ...
        structuredLogger.logInfo('Redis failover successful', {
          newPrimaryIndex: i
        });
    ...
    structuredLogger.logError('All Redis instances are unhealthy');
  }
  ...
  private async initializeSchema(): Promise<void> {
    ...
    try {
      ...
      structuredLogger.logInfo('Database schema initialized successfully');
    } catch (error) {
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      ...
      structuredLogger.logInfo('Resilient database manager shut down gracefully');
    } catch (error) {
      structuredLogger.logError('Error during database shutdown', {
        error: error instanceof Error ? error.message : String(error)
      });
    }
  }
